...
query <-  sprintf(
    "SELECT pbikes, bikes, slots, temperature, weather_type, UNIX_TIMESTAMP(updatetime) as updatetime FROM fit_precalculation_2 WHERE id=%d AND updatetime >= FROM_UNIXTIME(%d) AND updatetime <= FROM_UNIXTIME(%d)",
    stationID,
    from,
    to
)
