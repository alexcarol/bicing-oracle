...
query <-  sprintf(
    "SELECT pbikes, temperature, weather_type, UNIX_TIMESTAMP(updatetime) as updatetime FROM fit_precalculation WHERE id=%d AND updatetime >= FROM_UNIXTIME(%d) AND updatetime <= FROM_UNIXTIME(%d)",
    stationID,
    from,
    to
)
...
bikeFit <- randomForest(pbikes ~ updatetime + temperature + dayMoment + weekday + weather_type, data=data, importance=TRUE, ntree=100)
